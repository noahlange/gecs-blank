...
import { Vector2Array } from '@gecs/lib';
import { AOE, Direction } from '@gecs/types';
...
export function getCirclePoints(center: Vector2, radius: number): Vector2[] {
  ...
  return Array.from(points.keys(), point => ({
    x: point.x + center.x,
    y: point.y + center.y
  }));
}
...
export function getTargetAOE(source: WithDirection, aoe: AOE, range: number): Vector2[] {
